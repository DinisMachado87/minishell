...
/*   Updated: 2025/10/01 18:40:56 by dimachad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ms_heredoc(t_args *args, int offset, int expand)
{
	...
	pid = fork();
	if (pid == 0 && (!unique_tmp(&temp_file, TEMP_PREFIX, itoa(getpid()))
			|| !create_heredoc_file(args->tkns[offset], temp_file)))
		return (ERROR);
	else if (pid > 0 && (!unique_tmp(&temp_file, TEMP_PREFIX, itoa(pid))
